...
const Buscador = () => {
  ...
  useEffect(() => {
    if (input) {
      ...
      fetch(`http://localhost:3000/reparacion/${buscarPor}`)
        .then((res) => {
          if (!res.ok) {
        ...
        .catch((err) => {
          ...
          if (err.message === "No encontrado en reparacion") {
            fetch(`http://localhost:3000/instalacion/${buscarPor}`)
              .then((res) => {
                if (!res.ok) {
  ...
  <div className="flex flex-col items-center justify-center space-y-3 my-1 p-4 bg-white rounded-lg shadow-lg">
    <h2 className="text-2xl font-semibold text-gray-800">Buscar Cliente por </h2>
  ...
  <div className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-500">
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 18l6-6-6-6M10 18l-6-6 6-6" />
    </svg>
  </div>
</div>
